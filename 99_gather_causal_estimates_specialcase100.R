...
#load in gibbs results to eliminate non-convergence bros
results.coding.gibbs <- readRDS(paste0("output/gibbs_coding_",samplesize,"_all.rds"))
alpha.convergence <- results.coding.gibbs[[9]]
beta.convergence <- results.coding.gibbs[[10]]

bad <- which(alpha.convergence ==1 | beta.convergence>1)
...
allLists.coding <- lapply(iter[-c(202,209,416,549,550,bad)], readRDSx.coding) #remove ones that you know did not converge 
...
table.coding <- round(data.frame(truth=truth,
                           ...
                           bias = apply(apply(estimates.coding,1,prop_bias),1,mean),
                           abs_bias = abs(apply(estimates.coding,2,mean)-truth),
...
table.pl <-  data.frame(truth=round(truth,3),
                       ...
                       bias = round(apply(apply(estimates.pl,1,prop_bias),1,mean),3),
                       abs_bias = round(abs(apply(estimates.coding,2,mean)-truth),3),
                       variance = NA,
